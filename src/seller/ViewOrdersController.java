...
public class ViewOrdersController implements Initializable{
	...
	void getDetailsFromDatabase() {
		...
		Query<CustomerOrders> query = (Query<CustomerOrders>) session.createQuery("from CustomerOrders where isDelivered = 0");
	...
	void displayProducts(List<CustomerOrders> orders) {
		...
		try {
			...
			for(CustomerOrders custOrders : orders) {
				...
				loader.setLocation(getClass().getResource("ShowOrdersCard.fxml"));
				HBox hbox = loader.load();
				
				ShowOrdersCardController controller = loader.getController();
